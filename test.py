...
income = incomes[0,:2].reshape(-1, 1)
expense = expenses[0,:2]
print(income)
print(expense)
model = LinearRegression()
model.fit(income, expense)
new_income = np.array([[sum(incomes[0,:2])/len(incomes[0,:2])]])  # New week's income = 4597 yen
print(new_income)
predicted_budget = model.predict(new_income)

print("Predicted budget for the week: $", predicted_budget[0])